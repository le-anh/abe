...
void	encrypt_pdf(int index, int n_attrs)
{
	char	*cmd;
	int		i;
	...
	while (i++ < n_attrs)
	{
		cmd = wrap_cmd(NULL, cmd, get_str("attr", i));
		if (i + 1 <= n_attrs)
			cmd = wrap_cmd(NULL, cmd, ft_strdup(" and "));
	}

	cmd = wrap_cmd(NULL, cmd, ft_strdup("\" | cpabe-enc -k keys/pub_key original_files/"));
	cmd = wrap_cmd(cmd, get_str("file_", index), ft_strdup(".pdf"));
	system(cmd);
	free(cmd);

	cmd = ft_strdup("mv original_files/");
	cmd = wrap_cmd(cmd, get_str("file_", index), ft_strdup(".pdf.cpabe decrypted_files/"));
	cmd = wrap_cmd(cmd, get_str("file_", index), ft_strdup(".pdf.cpabe"));
	system(cmd);
	free(cmd);
}
